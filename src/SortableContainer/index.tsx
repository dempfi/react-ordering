  ...
  > {
    ...
    componentDidMount() {
      ...
      Promise.resolve(container).then((containerNode) => {
        ...
        events.end.forEach((name) =>
          this.container.addEventListener(name, this.handleEnd, false),
        );
        events.start.forEach((name) =>
          this.container.addEventListener(name, this.handleStart, false),
        );
        events.move.forEach((name) =>
          this.container.addEventListener(name, this.handleMove, false),
        );
    ...
    componentWillUnmount() {
      ...
      events.end.forEach((name) =>
        this.container.removeEventListener(name, this.handleEnd),
      );
      events.start.forEach((name) =>
        this.container.removeEventListener(name, this.handleStart),
      );
      events.move.forEach((name) =>
        this.container.removeEventListener(name, this.handleMove),
      );
    ...
    handleSortMove = (event: React.DragEvent) => {
    ...
    handleSortEnd = async (event: React.DragEvent) => {
    ...
    animateNodes() {
      ...
      const sortingOffset = {
        ...
        top:
          this.offsetEdge!.top + this.translate!.y + containerScrollDelta.top,
      };
      const {isKeySorting} = this.manager;
      ...
      for (let i = 0, len = nodes.length; i < len; i++) {
        ...
        if (outOfTheWayAnimationDuration) {
          setTransition(
            node,
            `transform ${outOfTheWayAnimationDuration}ms ${outOfTheWayAnimationEasing}`,
          );
          setTransition(
            node,
    ...
    dropAnimation() {
      return new Promise((resolve) => {
        ...
        const {edgeOffset, node: newNode} = this.manager.nodeAtIndex(
          this.newIndex,
        );
    ...
    keyLift = (event: React.KeyboardEvent) => {
      const {target} = event;
      const node = closest(target, (el) => el.sortableInfo != undefined);
